...
## Priors; 
for (t in 1:N_years){
  eps.R.nest[t] ~ dnorm(0, tau.R.nest)
}

mu.R.nest  ~ dunif(-5,5)
tau.R.nest <- pow(sigma.R.nest, -2)
sigma.R.nest ~ dunif(0, 15)
...
for (i in 1:N_nest){
  ...
  # Linear predictor
  R.nest_exp[i] <- exp(mu.R.nest + eps.R.nest[R_nest_year[i]])   
}

### Derive parameter: annual predictions for R.nest
for (t in 1:N_years){
  R.nest_year[t] <- exp(mu.R.nest + eps.R.nest[t])
}
...
## Derived; S_juv
for(t in 1:N_years){
S_juv[t] <- R_year[t] / R.nest_year[t]

}


} 
", fill=TRUE,file="Combined_M5.bugs")
