...
Posterior distributions for parameters estimated in three model fits to each of 10 simulated data sets are shown in @fig-sim-detection and @fig-sim-demo. Overall, the IDSM was able to corretly estimate both detection parameters and demographic rate parameters from all 10 simulated datasets without any systematic bias. The replicate runs for each dataset resulted in very similar posterior distributions, demonstrating that the models converged towards the same posterior distributions irrespective of starting values. Estimates of population sizes / densities were also adequate, and are presented in the Supplementary Materials (see folder SimCheck_byDataSet).
