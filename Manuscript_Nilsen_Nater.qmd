...
Based on simulated data, we assessed the models ability to estimate the underlying parameters. We run a total of 10 simulations, and the results are summarized in @fig-sim-detection and @fig-sim-demo. In general, the model was able to estimate both parameters of the detection model (Figure X) of the simulated data with little bias. This was true for all 10 simulations, and for underlying model parameters than span a wide variation in values. The full output from each simulation is available in the appendix.

![Model performance on simulated data. Upper panel depicts model parameters, and lower panels show derived estimated of effective strip width and mean detection probability. Dots represents true value, and density plot represent the posterior samples for each simulation](Figures/SimCheck_Detects_Temps.png){#fig-sim-detection}

![Posterior samples for the vital rates estimated based on simulated data. Upper panel show annual and seasonal survival probabilities, hyperparameters for the random effects model for recruitment (mean and sd), and lower panel show annual estimates of the recruitment rate. Dots represent true values, and density plots represents the posterior samples from each simulation](Figures/SimCheck_VRs_Temps.png){#fig-sim-demo}
...
-   Nevertheless, we did not detect a clear effect of rodent abundance (indexed by the occurrence of rodents on the transect lines) on recruitment rate. This might be somewhat surprising, given that such a pattern has been reported repeatedly in the literature (see e.g. @Bowler2020). However, the data on rodent abundance was heavily zero-inflated, and the annual variation in the index was rather small. \[A few sentences about fading rodent cycles, and perhaps a weakened link to ptarmigan dynamics\].
...
-   The model is flexible, and can be extended with additional auxiliary data, such as separate data on recruitment rates that could improve the precision of the demographic rate estimates. It is also possible to extent the model with additional covariates, to gain a better understanding of how species respond to environmental variation through the effects on vital rates.
...
CRN Lead - Updated and finalized model code; Lead - Modelling; Contributed - Wrote first version of ms
