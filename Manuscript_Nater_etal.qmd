...
All numerical results in the following are presented as median \[95% credible interval unless otherwise indicated. Posterior summaries (median, 95% credible interval, mean, standard deviation, coefficient of variation) for all main parameters are also provided in the supplementary file "PosteriorSummaries_byAreas.csv".
...
The auxiliary radio-telemetry data, rodent occupancy data, and all code including the "targets pipeline" can be found in the project's repository on GitHub:vv. The results presented in this paper were created using version 2.0 of the code (Zenodo citation TBA).
