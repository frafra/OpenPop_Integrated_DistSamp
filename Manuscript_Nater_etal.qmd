...
<!-- Maybe another sentence or two about the low survival following the peak, which could be either due to negative density-dependence leading to resource competition among ptarmigan, or due to the rodent peak allowing high recruitment also for predators, which then in turn will have increased impacts on all prey populations in the subsequent year? -->
...
Finally, including further data beyond the line transect surveys may be relevant in the future, and in particular in the context of informing and improving management of ptarmigan hunting. 
...
The most relevant source of data to be included into the IDSM framework in the near future, however, is harvest data. <!-- @ERLEND 1-2 sentence summary of what kind of harvest data is available, and at what spatial covarage and resolution --> As the IDSM framework is, in essence, an IPM, harvest can be modelled through partitioning of survival into cause-specific mortality in the process model and inclusion of relevant harvest data likelihoods [e.g. @gamelon2021; @nater2021]. Harvest effects on willow ptarmigan have been studied previously yet uncertainties persist [@sandercock_etal2011; aanes2002; @pedersen2004; @willebrand2001]. [...] <!-- @JAMES & Erlend: Maybe we can be a bit more specific about what the "uncertainties" are regarding ptarmigan harvest? Might be good to keep the "spatially explicit" perspective a bit, i.e. why would it be great to look at this with this kind of model, instead of just aggregate numbers across large regions? Also, maybe we should highlight that up-to-date (easilty repeatable) analyses are important because things can change, and harvest effects may interact with other (emergent) factors such as climate change and habitat degradation. Finally, it could also be nice to link this to some studies on other species... -->
