...
Recruitment rates varied between `r R.min$post.sum` (area "`r R.min$area.name`") and `r R.max$post.sum` (area "`r R.max$area.name`") and displayed a spatial pattern opposite to that 
of annual survival, i.e. lower recruitment rates co-occurring with higher survival rates and vice-versa (@fig-pVRB; @fig-paramCorrs B). Across all areas and years, average recruitment rate was `r R.hyper.mean$post.sum`. Unlike for survival, the model predicted similar magnitudes of spatial and temporal variation (random effect SDs of `r R.sd.spatial$post.sum` and `r R.sd.temporal$post.sum`, respectively), and about twice as much residual variation (`r R.sd.residual$post.sum`).  
...
Detection decay parameters, which determine detection probability in distance sampling surveys, varied across areas from between `r Det.min$post.sum` in "`r Det.min$area.name`" to `r Det.max$post.sum` in "`r Det.max$area.name`, resulting in detection probabilities over the transect sites (truncation distance = 200 m) from `r pDet.min$post.sum` to `r pDet.max$post.sum`, respectively. 
The global average detection decay was `r Det.hyper.mean$post.sum` (detection probability = `r pDet.hyper.mean$post.sum`), and in general, higher values were more common in the Southern half of the country than the Northern half (SF "Avg_detect_Map.pdf").
Variation in detection over time was modest (SF "TimeSeries_pDetect.pdf"), and among-year variation in detection decay (random effect log SD = `r Det.sd.temporal$post.sum`) was almost identical in magnitude to spatial (`r Det.sd.spatial$post.sum`) and residual (`r Det.sd.residual$post.sum`) variation. 
...
The relative importance of different components for explaining parameter variation differed among recruitment rate, survival probability, and detection decay (@fig-varDecomp).
The largest portion of variation in recruitment was attributed to residual variation (`r Rec_R` %), followed by spatial (`r Rec_A` %) and temporal (`r Rec_Y` %) variation. Rodent occupancy, which contains both a spatial and a temporal dimension, explained `r Rec_Cov` % of the total variation. 
For survival, there was large uncertainty in the estimated proportions of variance explained by different components. The model predicted similar potential contributions from temporal (`r Surv_Y` %) and residual (`r Surv_R` %) variation and suggested that spatial variation was only responsible for `r Surv_A` % of the total variance. 
Total variance in detection decay was attributed evenly to spatial, temporal, and residual variation at `r Det_A` %, `r Det_Y` %, and `r Det_R` %, respectively. 
