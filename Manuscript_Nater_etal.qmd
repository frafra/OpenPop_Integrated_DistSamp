...
<!-- @ERLEND: Any thoughts on what the status here is? Might there be other data available from beyond Lierne? Are there exciting future prospects e.g. linked to automated monitoring with cameras at a larger scale? -->
...
<!-- @ERLEND: related to this, I want to discuss the potential of including the not publicly available data. Is there an overview somewhere over what the coverage (in terms of municipalities) would be if we had access to all the Hønsefuglportalen data? This might matter not just for increasing sample size, but perhaps also making CAR modelling (spatial structure through neighborhood relations) possible -->
...
<!-- @JAMES: This one is for you. Some ruminations on how and why it would be great to formally model harvest, integration with hunting data, maybe eventually develop a tool to calculate quotas (I know this has been developed for other game species), etc. -->
...
<!-- @EVERYONE: Do we know of some good references for this? PRseumably we cannot cite our chapter in the Technical Manual yet :-P -->
...
TBA ...
Additionally, we would like to thank Bernardo Brandão Niebuhr dos Santos for his help with spatial data visualization. 
<!-- @ERLEND: you maybe know best all the people/institutions we need to thank here -->
<!-- @JAMES: should we be acknowledging your fullbright scholarship which made the collab possible? If so, please add details. --> 
...
The auxiliary radio-telemetry data, rodent occupancy data, and all code including the "targets pipeline" can be found in the project's repository on GitHub: <https://github.com/ErlendNilsen/OpenPop_Integrated_DistSamp>.
The results presented in this paper were created using version 2.0 of the code (Zenodo citation TBA). 
