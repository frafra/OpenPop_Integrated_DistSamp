...
$$
\begin{aligned}
  D_{juv, x, t+1} & = D_{ad, x, t+1} * R_{x, t+1} \\
  D_{ad, x, t+1} & = S_{x, t} * (D_{juv, x, t} + D_{ad, x, t})  
\end{aligned}
$$

Here, $D_{juv, x, t}$ and $D_{ad, x, t}$ are the densities of juvenile and adult ptarmigan in area $x$ in year $t$, respectively. Both juveniles and adult survive from year $t$ to $t+1$ with an area- ($x$) and year- ($t$) specific survival probability $S_{x, t}$, and survivors produce the next generation of juveniles according to an area- and year-specific recruitment rate ($R_{x,t}$).
