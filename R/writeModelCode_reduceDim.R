...
writeModelCode_reduceDim <- function(survVarT, telemetryData){
  ...
  IDSM.code <- nimble::nimbleCode({
    ...
    # meanDens[x, a, j, t] = meanDens of age class a individuals in site j of area x in year t
    ...
    for(x in 1:N_areas){
      ...
      #------------------------------------#
      # Initial population density (t = 1) #
      #------------------------------------#
      
      ## Adult densities
      meanDens[x, 2, 1] <- Mu.D1[x]
      
      ## Juvenile densities
      if(R_perF){
        meanDens[x, 1, 1] <- (meanDens[x, 2, 1]/2)*R_year[x, 1] 
      }else{
        meanDens[x, 1, 1] <- meanDens[x, 2, 1]*R_year[x, 1]
      }

      
      #-------------------------------#
      # Population dyensity for t > 1 #
      #-------------------------------#
      
      for(t in 2:N_years){
        
        ## Adult densities
        meanDens[x, 2, t] <- sum(meanDens[x, 1:N_ageC, t-1])*S[x, t-1] 
        
        ## Juvenile densities
        if(R_perF){
          meanDens[x, 1, t] <- (meanDens[x, 2, t]/2)*R_year[x, t] 
        }else{
          meanDens[x, 1, t] <- meanDens[x, 2, t]*R_year[x, t]
        }
      }
      
      #---------------------------------#
      # Transect-level population sizes #
      #---------------------------------#

      ## Adult and juvenile numbers
      for(j in 1:N_sites[x]){
        for(t in 1:N_years){
          N_exp[x, 1:N_ageC, j, t] <- meanDens[x, 1:N_ageC, t]*L[x, j, t]*W*2
          #for(a in 1:N_ageC){
          #  N_exp[x, a, j, t] ~ dnorm(meanDens[x, a, t]*L[x, j, t]*W*2, sd = sigma.D[x])
          #}
        }
      }
