...
simulateInits_reduceDim <- function(nim.data, nim.constants, R_perF, survVarT, fitRodentCov, initVals.seed){
  ...
  meanDens <- array(0, dim = c(N_areas, N_ageC, N_years))
  ...
  for(x in 1:N_areas){
    ...
    if(R_perF){
      meanDens[x, 1, 1] <- (meanDens[x, 2, 1]/2)*R_year[x, 1] # Juveniles 
    }else{
      meanDens[x, 1, 1] <- meanDens[x, 2, 1]*R_year[x, 1] # Juveniles
    }
    
    for(j in 1:N_sites[x]){
      
      N_exp[x, 1:N_ageC, j, 1] <- meanDens[x, 1:N_ageC, 1]*L[x, j, 1]*W*2
    }
  }
  ...
  for(x in 1:N_areas){
    for(t in 2:N_years){
      ...
      meanDens[x, 2, t] <- sum(meanDens[x, 1:N_ageC, t-1])*S[x, t-1] # Adults
      
      if(R_perF){
        meanDens[x, 1, t] <- (meanDens[x, 2, t]/2)*R_year[x, t] # Juveniles 
      }else{
        meanDens[x, 1, t] <- meanDens[x, 2, t]*R_year[x, t] # Juveniles
      }
